...
type AutoPhase = 'round-intro' | 'card-enter' | 'card-result' | 'round-end'
...
export default function TestArena({
  ...
  // Track which matches are KO'd (stop showing new cards for them)
  ...
  useEffect(() => {
    ...
    const delay: Record<AutoPhase, number> = {
      'round-intro': 2000,
      'card-enter': 800,
      'card-result': 1500,
      'round-end': 0,
    ...
    timerRef.current = setTimeout(() => {
      switch (autoPhase) {
        ...
        case 'card-enter':
          // Apply damage for current card across ALL matches simultaneously
          setDisplayHp((prev) => {
            const updated = { ...prev }
        ...
        case 'card-result':
          ...
          precomputed.matches.forEach((match, mi) => {
            ...
            // Need to read from latest displayHp
            // We'll check in the next render, for now mark potential KOs
          })

          // Check if all matches are done or KO'd, or we've done 5 cards
          if (cardIdx >= 4) {
  ...
  // Detect KOs after HP updates
  useEffect(() => {
    if (!precomputed || autoPhase !== 'card-result') return
    const newKos = new Set(matchKo)
    let changed = false
    precomputed.matches.forEach((match, mi) => {
      if (newKos.has(mi)) return
      const h1 = displayHp[match.player1Id] ?? 0
      const h2 = displayHp[match.player2Id] ?? 0
      if (h1 <= 0 || h2 <= 0) {
        newKos.add(mi)
        changed = true
      }
    ...
    // If all matches are KO'd, end round early
    if (newKos.size === precomputed.matches.length && autoPhase === 'card-result') {
      clearTimer()
      timerRef.current = setTimeout(() => setAutoPhase('round-end'), 1500)
    }
  ...
  // Get all player IDs currently fighting
            ...
            <div className="space-y-3 animate-[fadeIn_0.5s_ease-out]">
              ...
              {precomputed.matches.map((m, i) => (
                <div key={i} className="rounded-lg border border-zinc-800 bg-zinc-900 py-3 text-center">
                  <span className={m.player1Id === userId ? 'text-amber-400 font-bold' : 'text-white font-bold'}>
                    {getPlayer(m.player1Id)?.name}
                  </span>
                  <span className="mx-3 text-zinc-600 font-black">VS</span>
                  <span className={m.player2Id === userId ? 'text-amber-400 font-bold' : 'text-white font-bold'}>
          ...
          {autoPhase === 'round-end' && precomputed && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6 text-center animate-[fadeIn_0.5s_ease-out]">
              ...
              {aliveCount() <= 1 ? (
                ...
                <button onClick={() => { setPrecomputed(null); setAutoPhase(null); startNextRound() }}
